...
def log_in(request):
    ...
    orjwt = jwt.encode({'username': username, 'time': time.strftime(
        "%Y%m%d-%H%M%S")}, "secret", algorithm="HS256")
...
def get_task_basic_info(request):
    ...
    if tag == 'receiveTask':
        ...
        user_current_problem_finish_number, user_received_total_problem_number,\
            task_status = get_user_received_problem_info(
                receiver_name, eval(sort_choice), eval(task_index))
...
def finish_daily_task(request):
    ...
    finish_task_one, finish_task_two = update_daily_task_info(
        username, eval(index))
    res = {
        'status': 'ok',
...
def get_user_released_task_info(request):
    ...
    username = query_dict.get("username", "")
    page_number = query_dict.get("pageNumber", "")
    # 0是所有，1是暂未发布 2是发布但未结束 3是已结束
    sort_choice = query_dict.get("sortChoice", "")
...
def withdraw_money(request):
    ...
    username = query_dict.get("username", "")
    money_number = eval(query_dict.get("money", ""))
    is_success, current_donut_number = user_withdraw_money(
        username, money_number)
    res = {
        'status': 'ok',
...
def release_task(request):
    ...
    if request_type == 'application/json':
        request_body = json.loads(request.body)
        username, t_id, t_release_mode, sub_donut_number = create_task(
            request_body)
        ...
        path_name = os.path.join(TASK_COVER_SAVE_PATH,
                                 username + current_time + file_format)
...
def final_submit(request):
    ...
    pass_insertion_test, is_punish, today_violation_number,\
        ...
        get_donut_number, now_donut_number = final_submit_answer(
            username, task_id)
...
def add_reported_task(request):
    ...
    if image == None:
        ...
        path = os.path.join("./media/report_image",
                            time.strftime("%Y%m%d-%H%M%S") + image.name)
...
def get_feedback(request):
    ...
    for i in range(len(feedback_list)):
        ...
        with open(path_name, 'rb') as f:
            data = f.read()
            feedback_list_dict[i]['image_url'] = bytes.decode(
...
def interrupt_task(request):
    query_dict = request.GET
    task_id = query_dict.get("taskId", "")
    left_problem_number, add_donut_number = poster_interrupt_current_task(
        task_id)
    res = {
        'status': 'ok',
...
def post_task_immediately(request):
    query_dict = request.GET
    task_id = query_dict.get("taskId", "")
    is_succeed, sub_donut_number, current_donut_number = force_release_task(
        task_id)
    res = {
        'status': 'ok',

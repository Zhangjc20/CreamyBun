...
export default {
  ...
  mounted() {
    ...
    if (this.type == 2 || this.type == 3) {
      this.streamSrc =
        "http://101.42.118.80:8000/stream_video/" +
        this.materialInfo["filePath"].replace(/\//g, "<").replace(/\\/g, "<");
    }
    if (this.type == 3) {
    ...
    if (this.type == 0) {
      ...
      this.image.src =
        "http://101.42.118.80:8000" + this.materialInfo["filePath"].substr(1);
    ...
    axios
      .get("http://101.42.118.80:8000/perform_problem_material/", {
  ...
  methods: {
    getVideoStream() {
      axios
        .get("http://101.42.118.80:8000/stream_video/", {
          params: this.materialInfo,
          responseType: "blob",
        })
        .then((res) => {

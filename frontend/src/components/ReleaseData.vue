<template>
  <el-header class="header-style">
    <el-row>
      <el-col :span="8">
        ...
        <span class="header-title"> {{ this.materialTypeName }}任务 </span>
      </el-col>
      <el-col :span="16">
  ...
  <el-main style="height: calc(100vh - 220px); margin-top: 20px">
                        ...
                        <template #dropdown>
                          <el-dropdown-menu>
                            <el-dropdown-item
                              ...
                              @click="
                                moveQuestionUpward(scope.row, scope.$index)
                              "
                              >上移</el-dropdown-item
                            >
                            <el-dropdown-item
                              icon="el-icon-remove"
                              @click="moveQuestionDown(scope.row, scope.$index)"
                              >下移</el-dropdown-item
                            >
                            <el-dropdown-item
                              icon="el-icon-remove"
                              @click="editQuestion(scope.row, scope.$index)"
                              >编辑</el-dropdown-item
                            >
                            <el-dropdown-item
          ...
          <el-row style="margin-left: 40px; margin-right: 20px">
            <el-main class="main-style">
              <el-row style="height: 50px">
                <span
                  class="header-title"
                  style="margin: auto, auto, auto, 20px; font-size: 17px"
                  >请上传任务封面</span
                >
              </el-row>
    ...
    <el-main class="main-style">
      <el-row style="height: 50px">
        <span class="header-title"> 发布信息 </span>
      </el-row>
      <el-form :model="form" label-width="100px" class="change-form">
        <el-row style="height: 50px">
          <el-form-item label="任务星级" :required="true">
            <el-select
              v-model="form.starRank"
              class="m-2"
              placeholder="请选择星级"
            >
              <el-option
        ...
        <el-row style="height: 50px">
          <el-form-item label="题目奖励" :required="true">
            <el-input
              v-model="form.singleBonus"
              placeholder="请输入单个题目的奖励"
              type="number"
              @blur="form.singleBonus = Number(form.singleBonus)"
        ...
        <el-row style="height: 50px">
          <el-form-item label="领取人数" :required="true">
            <el-input
              v-model="form.receiverNum"
              placeholder="请输入本任务的领取人数"
              type="number"
              @blur="form.receiverNum = Number(form.receiverNum)"
              oninput="value=value.replace(/[^\d]/g,'')"
            />
          </el-form-item>
        </el-row>
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        //questionNum taskLevel username
        taskName: "",
        poster: "",
        description: "",
        questionType: "",
  ...
  methods: {
    ...
    finalCheck() {
      if (this.form.taskName == "") {
        ...
        this.form.releaseMode == "定时发布" &&
        (this.form.startLine1 == "" || this.form.startLine2 == "")
      ) {
        this.$message({
          message: "您尚未填写完成发布日期！",
          type: "error",
        ...
        this.donutList[this.form.starRank - 1] > this.form.singleBonus
      ) {
        this.$message({
          message:
            "您的设置的单个题目奖励过低！最低为：" +
            this.donutList[this.form.starRank - 1].toString() +
            "个甜甜圈！",
          type: "error",
        });
        ...
        for (var i = 0; i < this.questionList.length; i++) {
          if (this.questionList[i]["questionType"] == 3) {
            ...
            if (
              tempTarget > this.$refs.MyMaterialUpload.fullList.length ||
              this.$refs.MyMaterialUpload.fullList[tempTarget][0]["fileType"] !=
                0
            ) {
              this.$message({
                message:
        ...
        if (
          (tempLen - this.$refs.MyMaterialUpload.testList.length) /
            this.form.receiverNum <
          1
        ) {
          this.$message({
        ...
        var deadLine = new Date(
          this.form.deadLine1 + " " + this.form.deadLine2
        );
        var date = new Date();
        if (startLine < date) {
          this.$message({
    ...
    async preCheck() {
      await axios
        .get("http://101.42.118.80:8000/get_release_info/", {
          params: {
    ...
    async finalSubmit() {
      ...
      this.form.problemTotalNum =
        this.$refs.MyMaterialUpload.fullList[0].length;
      ...
      await axios
        .post("http://101.42.118.80:8000/release_task/", {

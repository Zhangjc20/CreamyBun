<template>
  ...
  <el-main style="height:calc(100vh - 220px);margin-top: 20px;">
    <el-main class="main-style" style="margin-top:  0px;">
      ...
      <el-row>
          ...
          <el-form :model="form" label-width="100px" class="change-form">
            ...
            <el-row style="height: 80px;">
              <el-form-item label="任务描述" :required="true">
                <el-input v-model="form.description" :rows="3" type="textarea" placeholder="请输入任务描述" resize="none"
                  style="width: 300px;" />
              </el-form-item>
            </el-row>
            <el-row style="height: 50px;">
              <el-form-item label="题型" :required="true" style="margin-top: 10px;">
                <el-select v-model="form.questionType" class="m-2" placeholder="请选择题型">
                  <el-option @click="showQuestionPage(item)" v-for="item in options" :key="item.value"
                    :label="item.label" :value="item.value" />
                </el-select>
              </el-form-item>
            </el-row>
            <el-row>
              <component :is="componentName" :multiple="multiple" :editingQuestion="editingQuestion"
                :questionList="questionList" :newOrEdit="newOrEdit" @questionSubmit="addQuestion">
              </component>
            ...
            <el-row>
              <el-main class="main-style" style="margin-right: 40px;">
                ...
                <el-table :data="questionList" height="200" :style="table" class="customer-table">
                        ...
                        <template #dropdown>
                          <el-dropdown-menu>
                            <el-dropdown-item icon="el-icon-s-order"
                              @click="moveQuestionUpward(scope.row, scope.$index)">上移</el-dropdown-item>
                            <el-dropdown-item icon="el-icon-remove"
                              @click="moveQuestionDown(scope.row, scope.$index)">下移</el-dropdown-item>
                            <el-dropdown-item icon="el-icon-remove"
                              @click="editQuestion(scope.row, scope.$index)">编辑</el-dropdown-item>
                            <el-dropdown-item icon="el-icon-download"
                              @click="deleteQuestion(scope.row, scope.$index)">删除</el-dropdown-item>
                          </el-dropdown-menu>
                        </template>
        ...
        <el-col :span="12" style="border-left: 1px solid #999999;">
          ...
          <el-row>
            <MaterialUpload ref="MyMaterialUpload" :username="username" :questionList="questionList"
      ...
      <el-form :model="form" label-width="100px" class="change-form">
        ...
        <el-row style="height: 50px;">
          <el-form-item label="题目奖励" :required="true">
            <el-input v-model="form.singleBonus" placeholder="请输入单个题目的奖励" type="number"
              @blur="form.singleBonus = Number(form.singleBonus)" oninput="value=value.replace(/[^\d]/g,'')" />
          </el-form-item>
        </el-row>
        <el-row style="height: 50px;">
          <el-form-item label="领取人数" :required="true">
            <el-input v-model="form.receiverNum" placeholder="请输入本任务的领取人数" type="number"
              @blur="form.receiverNum = Number(form.receiverNum)" oninput="value=value.replace(/[^\d]/g,'')" />
          </el-form-item>
        </el-row>
        <el-row style="height: 50px;">
        ...
        <el-row v-if="form.releaseMode == '定时发布'" style="height: 50px;">
          <el-form-item label="发布时间" :required="true">
            <el-col :span="11">
              <el-date-picker v-model="form.startLine1" type="date" placeholder="请选择发布日期" style="width: 100%"
            ...
            <el-col :span="11">
              <el-time-picker v-model="form.startLine2" placeholder="请选择发布时间" style="width: 100%"
                value-format="HH:mm:ss" :disabled-date="disabledDate" />
            </el-col>
          </el-form-item>
        </el-row>
        <el-row style="height: 50px;">
          <el-form-item label="截止时间" :required="true">
            <el-col :span="11">
              <el-date-picker v-model="form.deadLine1" type="date" placeholder="请选择截止日期" style="width: 100%"
                value-format="YYYY-MM-DD" :disabled-date="disabledDate" />
            </el-col>
            <el-col :span="2" class="text-center">
              <span class="text-gray-500"> :</span>
            ...
            <el-col :span="11">
              <el-time-picker v-model="form.deadLine2" placeholder="请选择截止时间" style="width: 100%" value-format="HH:mm:ss"
